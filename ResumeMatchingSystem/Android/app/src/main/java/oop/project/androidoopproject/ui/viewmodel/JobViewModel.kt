...
class JobViewModel : ViewModel() {
    ...
    /** Loads all open jobs. Null/blank params return everything. */
    fun searchJobs(title: String? = null, location: String? = null, shift: String? = null) {
        viewModelScope.launch {
            ...
            try {
                ...
                if (res.isSuccessful)
                // Backend returns List directly — no .content needed anymore
                    _jobsState.value = UiState.Success(res.body() ?: emptyList())
                else
                    _jobsState.value = UiState.Error("Failed to load jobs. (${res.code()})")
            } catch (e: Exception) {
